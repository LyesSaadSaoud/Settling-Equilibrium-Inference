...
def fig1_environment():
    ...
    ax.set_title('Controlled non-convex validity environment\n'
                 'Two designated valid modes whose arithmetic mean is invalid',
                 fontweight='bold')
    fig.tight_layout()
...
def fig3_settling_dynamics(settling_result):
    ...
    fig.suptitle('Settling Dynamics: Escape from an Invalid Mean and Convergence\n'
                 r'$s^{(k+1)} = s^{(k)} - \eta\,\nabla_s E(s^{(k)}, c)$   '
...
def fig4_energy_landscape():
    ...
    fig.suptitle('Illustrative Reduced Energy Landscape: Non-Convex Validity Structure',
                 fontweight='bold')
    fig.tight_layout()
...
def fig5_phase_transition():
    ...
    fig.suptitle('Figure 5 — Phase Transition: Settling Bifurcation vs. Mean Collapse',
                 fontweight='bold')
    fig.tight_layout()
...
def fig8_energy_convergence(settling_result, diffusion_result, ebm_result):
    ...
    ax.set_title('Figure 8 — Energy Convergence Curves\n'
                 'Settling converges monotonically (Lemma 1); '
                 'others oscillate or stagnate',
                 fontweight='bold')
